...
//        encrypt = true;
    ...
    public static void fileRecognize() throws Exception {
        ...
        if (!file.exists()) {
            ...
            System.err.println("Haven't write right: " + filePath);
            LerVarExecute.interrupt();
            return;
        }
    ...
    public static void _LerVarFileHeadWriter() throws Exception {
        ...
        for (int i = 0; i <= CHOOSE_VERIFY_CODE[0].length - 1; i++) {
            System.out.print(CHOOSE_VERIFY_CODE[0][i]);
        }
        String verifyCodeChoice = new Scanner(System.in).nextLine();
        if (verifyCodeChoice.equals("0")) {
            interrupt();
        } else {
            verifyCode =
